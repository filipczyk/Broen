...
**Source (input):** Broen canonical JSON — a simplified camelCase JSON schema (`CreateInvoiceRequest` DTO in `Core/DTOs/`). Not UBL, not CII, not raw EN 16931. Designed for easy API consumption. Reference example: `request.json` at repo root.

**Target (output):** UBL 2.1 XML conforming to **XRechnung 3.0** (German CIUS of EN 16931), delivered via **Peppol BIS Billing 3.0**.
- CustomizationID: `urn:cen.eu:en16931:2017#compliant#urn:xeinkauf.de:kosit:xrechnung_3.0`
- ProfileID: `urn:fdc:peppol.eu:2017:poacc:billing:01:1.0`
- Generated by `UblInvoiceTransformer` using `System.Xml.Linq` (XDocument/XElement) — never string concatenation
- Validated against KoSIT validator (0 errors, 0 warnings as of 2026-03-17)
...
3. **TransformationConsumer** → Broen JSON → UBL 2.1 XML (XRechnung 3.0) → publishes `InvoiceTransformed`
